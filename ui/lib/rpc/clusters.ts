...
export interface SuspendKustomizationReq {
    contextname?: string;
    namespace?: string;
    kustomizationname?: string;
}

interface SuspendKustomizationReqJSON {
    contextName?: string;
    namespace?: string;
    kustomizationName?: string;
}
...
const SuspendKustomizationReqToJSON = (m: SuspendKustomizationReq): SuspendKustomizationReqJSON => {
	if (m === null) {
		return null;
	}
...
export interface SuspendKustomizationRes {
    kustomization?: Kustomization;
}

interface SuspendKustomizationResJSON {
    kustomization?: KustomizationJSON;
}



const JSONToSuspendKustomizationRes = (m: SuspendKustomizationRes | SuspendKustomizationResJSON): SuspendKustomizationRes => {
    if (m === null) {
		return null;
	}
    return {
...
export interface Clusters {
    ...
    suspendKustomization: (suspendKustomizationReq: SuspendKustomizationReq) => Promise<SuspendKustomizationRes>;
...
export class DefaultClusters implements Clusters {
    ...
    suspendKustomization(suspendKustomizationReq: SuspendKustomizationReq): Promise<SuspendKustomizationRes> {
        const url = this.hostname + this.pathPrefix + "SuspendKustomization";
        let body: SuspendKustomizationReq | SuspendKustomizationReqJSON = suspendKustomizationReq;
        if (!this.writeCamelCase) {
            body = SuspendKustomizationReqToJSON(suspendKustomizationReq);
        }
        return this.fetch(createTwirpRequest(url, body, this.headersOverride)).then((resp) => {
            if (!resp.ok) {
                return throwTwirpError(resp);
            ...
            return resp.json().then(JSONToSuspendKustomizationRes);
        });
    }
